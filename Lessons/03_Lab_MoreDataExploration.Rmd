...
# Lists ---- 
list1 <- list(name = "Maria", mynumbers = vector1, mymatrix = matrix1, age = 5.3);  #; allows you to do two different r statements in one line
list1[[2]]
list1['mynumbers'] #same as above line

# Data Frames ---- a dataframe can be thought of as a list of vectors (each column is a vector and all columns/vectors are the same length)
d <- c(1, 2, 3, 4) # What type of vector? numberic
...
names(dataframe1) <- c("ID","Color","Passed"); View(dataframe1) #updates column names in dataframe
...
> Answer: the environment tab states their data type (char, num, int, etc). There is also a data section and a values section. The data section has things like dataframes and matrixes (multiple dimensions) and the values section has single dimension strings and such and single values

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer: nothing (no matches), but if you remove the "\" it has matches
...
```{r DataExercise-1a}
high_temp <- c(70, 71, 63, 50, 43, 59, 60, 66, 67, 61)
low_temp <- c(48, 58, 46, 37, 35, 39, 49, 48, 51, 32)
```
...
```{r DataExercise-1b}
FtoC <- function(temp) {(temp - 32)*(5/9)}
high_temp_C <- FtoC(high_temp)
low_temp_C <- FtoC(low_temp)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
temperatures <- data.frame("HighTemps_F" = high_temp, "LowTemps_F" = low_temp, "HighTemp_C" = high_temp_C, "LowTemp_C" = low_temp_C)
```
...
```{r DataExercise-1d}
summary(temperatures$HighTemps_F) #calling for a single column
sd(temperatures$HighTemps_F)

summary(temperatures) #calling for the whole data frame
sd(temperatures) #error :(
```
