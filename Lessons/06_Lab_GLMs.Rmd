...
plot1 <- ggplot(EPAair, aes(x=Ozone, fill=as.factor(Year))) +
  ...
  labs(fill="", x="Ozone AQI Value", y="Density")
print(plot1)
...
ozone.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(ozone.regression)

ozone.regression2 <- lm(data=EPAair, PM2.5 ~ Ozone)
summary(ozone.regression2)
...
plot4 <- ggplot(EPAair, aes(x=Ozone, y=PM2.5)) +
  geom_point() +
...
plot5 <- 
  ...
  drop_na()
corr_plot5 <- cor(plot5)
corrplot(corr_plot5, method = "ellipse")
corrplot.mixed(corr_plot5, upper = "ellipse")
...
PM2.5_model <- lm(data = EPAair, PM2.5 ~ Ozone + Year + Month + mean_Lat + mean_Lng)
step(PM2.5_model)
...
PM2.5_model2 <- lm(data = EPAair, PM2.5 ~ Ozone + Month + mean_Lat + mean_Lng)
summary(PM2.5_model2)
```
