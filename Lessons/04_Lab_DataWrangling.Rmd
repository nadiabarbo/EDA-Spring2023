...
NTL.phy.data.E1 <- filter(NTL.phys.data, year4 == 1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phy.data.E2 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))

```
Question: Why don't we filter using row numbers?

> Answer: Because row numbers can change/are not concrete based on who creates that data. Using row names is more concrete.
...
NTL.phys.data.E3 <- NTL.phys.data %>%
  mutate(month = format(as.Date(sampledate, format="%Y/%m/%d"),"%m")) %>%
  filter(lakename == "Tuesday Lake" & year4 %in% c(1990:1999) & month == "07")
#OR
NTL.phys.data.E3 <- 
  NTL.phys.data %>%
...
NTL.phys.data.E5 <- pivot_longer(NTL.phys.data, cols = irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")
#OR
NTL.phys.data.E5 <- 
  NTL.phys.data %>%
  pivot_longer(irradianceWater:irradianceDeck, names_to = "Irradiance_Type", values_to = "Irradiance_Value")
